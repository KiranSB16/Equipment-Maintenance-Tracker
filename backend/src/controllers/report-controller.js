...
reportCltr.equipmentStatus = async (req, res) => {
  try {
    const equipments = await Equipment.find().select(
      "name type status lastMaintenanceDate"
    );
    ...
    return res
      .status(500)
      .json({ message: "Error generating equipment report" });
  }
};
...
reportCltr.workOrderSummary = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error generating work order report" });
  }
};
...
reportCltr.technicianWorkload = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error generating technician workload report" });
  }
};
